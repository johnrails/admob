...
private
  ...
  class Defaults
    class << self
      attr_accessor :publisher_id, :analytics_id, :encoding, :timeout, :raise_exceptions, :cookie_domain, :cookie_path,:ad_request
    end
  end
  ...
  def self.build_post_data(request, session_id, params)
    ...
    post_data = {
      ...
      'v'         => PUBCODE_VERSION,
      'o'         => request.cookies['admobuu'] || request.env['admobuu'],
      's'         => publisher_id,
      'a'         => analytics_id,
      'ma'        => params[:markup],
